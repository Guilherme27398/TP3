...
def extract_and_filter_tables_with_captions(pdf_path, tables, headings=["Summary of evidence", "Recommendations"]):
    doc = fitz.open(pdf_path)  # Correção garantida
...
def get_relevant_pages(pdf_path):
    ...
    for i, page_text in enumerate(text_pages):
        if start_page is None and "2. FREQUÊNCIA DAS ATIVIDADES LETIVAS" in page_text:
            start_page = i + 1  # i + 1 porque as páginas começam em 1
        if end_page is None and "5. DISPOSIÇÕES FINAIS" in page_text:
            end_page = i + 1

    if start_page is not None and end_page is not None:
        return start_page, end_page
    else:
        raise ValueError("Não foi possível encontrar as secções 'Frequência' ou 'Disposições Finais' no documento.")
